...
const PrintPanelMonth = () => {
  ...
  useEffect(() => {
    setTimeout(() => {
      window.print();
    }, 1500); // Delay to ensure chart/table renders
  }, []);

  if (!state) return <div>No data to print.</div>;

  const { year, month, tableData, issueCounts } = state;
  ...
  return (
    <div ref={printRef} className="p-10 font-montserrat">
      ...
      <h1 className="text-2xl font-bold text-center mb-6 font-montserrat">
        Summary Report for the – {month} {year}
      </h1>
...
export default PrintPanelMonth;
